...
// rotate left-right
glm::vec3 rotateLR(glm::vec3 v, float radians) {
	return v * glm::transpose(
		glm::mat3(1, 0, 0,
...
// rotate front-back
glm::vec3 rotateFB(glm::vec3 v, float radians) {
	return v * glm::transpose(
		glm::mat3(cos(radians), 0, sin(radians),
			0, 1, 0,
...
void Laborator8::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color)
{
	...
	// trimitere isSpotlight
	int spot_position = glGetUniformLocation(shader->program, "is_spotlight");
	glUniform1i(spot_position, isSpotlight);

	// [bonus] - trimitere unghi cut_off

	int cut_off_location = glGetUniformLocation(shader->program, "cut_off_angle");
	glUniform1f(cut_off_location, cut_off_angle);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_UP)) {
			lightDirection = rotateLR(lightDirection, RADIANS(150 * deltaTime));
		}
		...
		if (window->KeyHold(GLFW_KEY_LEFT)) {
			lightDirection = rotateFB(lightDirection, -RADIANS(150 * deltaTime));
		}
