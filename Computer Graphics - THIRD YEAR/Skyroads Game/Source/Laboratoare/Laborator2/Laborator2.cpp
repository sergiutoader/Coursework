...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		// Vertices and Indices for pyramid object
		vector<VertexFormat> vertices_2
		{
			VertexFormat(glm::vec3(0, 0, 0), glm::vec3(1, 0, 1)),
			VertexFormat(glm::vec3(2, 0, 0), glm::vec3(1, 0, 1)),
			VertexFormat(glm::vec3(1, 0, sqrt(3)), glm::vec3(1, 0, 1)),
			VertexFormat(glm::vec3(1, 2 * sqrt(6) / 3, sqrt(3) / 3), glm::vec3(1, 0, 1)),
		};

		vector<unsigned short> indices_2 = {
			0, 1, 2,
			0, 3, 1,
			0, 2, 3,
			1, 3, 2
		...
		// Exercitiul 6
		vector<VertexFormat> vertices_3
		{
			VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0.5, 0, 1)),
			VertexFormat(glm::vec3(0, 0, 1), glm::vec3(0.5, 0, 1)),
			VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0.5, 0, 1)),
			VertexFormat(glm::vec3(1, 0, 1), glm::vec3(0.5, 0, 1)),
		};

		vector<unsigned short> indices_3 = {
			0, 1, 2,
			1, 2, 3
		};
		...
		// tetraedru regulat
		Mesh* pyramid = CreateMesh("pyramid", vertices_2, indices_2);
		// patrat
		Mesh* square = CreateMesh("square", vertices_3, indices_3);
	}
}
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	// Randare obiecte create de mine
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(-3.0f, 0.5f, 0), glm::vec3(0.25f));
	RenderMesh(meshes["pyramid"], shaders["VertexColor"], glm::vec3(-6.0f, 0.5f, 0), glm::vec3(0.25f));
	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(-9.0f, 0.5f, 0), glm::vec3(1.0f));

	RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(-12.0f, 0.5f, 0), glm::vec3(1.0f));
...
void Laborator2::OnKeyPress(int key, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_F2) == true) {
		if (cullFace == GL_BACK) {
			cullFace = GL_FRONT;
		}
		else {
