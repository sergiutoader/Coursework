...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab9"], modelMatrix, true, mapTextures["earth"]);
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab9"], modelMatrix, false, mapTextures["crate"]);
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab9"], modelMatrix, false, mapTextures["random"]);
	}

	{
		...
		RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix, false, mapTextures["grass"]);
	}

	{
		...
		RenderSimpleMesh(meshes["bamboo"], shaders["ShaderLab9"], modelMatrix, false, mapTextures["bamboo"]);
	}
	...
	// ex 8
	{
		...
		RenderSimpleMesh(meshes["quad"], shaders["ShaderLab9"], modelMatrix, false, mapTextures["crate"], mapTextures["random"]);
	}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, bool rotating, Texture2D* texture1, Texture2D* texture2)
{
	...
	// rotatie
	int location_rotating = glGetUniformLocation(shader->program, "rotating");
	if (rotating) {
		glUniform1i(location_rotating, 1);
	}
	else {
		glUniform1i(location_rotating, 0);
	}
...
Texture2D* Laborator9::CreateRandomTexture(unsigned int width, unsigned int height)
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER); // modul de wrapping pe orizontala
